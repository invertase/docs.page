...
import { Children, ComponentProps, isValidElement, useState } from 'react';
...
export function CodeGroup(props: CodeGroupProps) {
  ...
  const groups =
    Children.map(props.children, child => {
      ...
      if (!child || !isValidElement(child) || !child.props?.html?.startsWith('<pre')) {
        return null;
      }
  ...
  const languages = groups.map(group => String(group.props.language || 'text'));
  ...
  const copy = groups.find(group => group.props.language === active)?.props.raw;
      ...
      <div className="h-10 flex items-center bg-black text-xs border-b border-white/20 font-display tracking-wider">
        ...
        <div className="flex items-center px-3  ">
          {languages.map(language => (
            <button
              key={language}
              className={cn('text-white/70 hover:bg-white/10 px-2 py-1.5 rounded-md', {
        ...
        {groups.map(group => {
          return (
            <div
              ...
              className={cn('[&>pre]:p-4 [&>pre]:overflow-x-auto text-sm leading-6', {
