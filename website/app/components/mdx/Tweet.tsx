...
declare global {
  interface Window {
    twttr: {
      widgets: {
        createTweet: (tweetId: string, el: HTMLElement, options?: unknown) => Promise<void>;
      };
    };
  }
}
...
export function Tweet(props: TweetProps) {
  ...
  // String is legacy, boolean is new.
  const cards = props.cards === 'hidden' ? 'hidden' : !props.cards ? 'hidden' : '';
  const conversation = props.conversation === 'none' ? 'none' : !props.conversation ? 'none' : '';
  ...
  }, [ref, props.id, loaded, cards, conversation]);
