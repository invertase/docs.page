...
export async function GET(request: Request) {
  ...
  const params = JSON.parse(
    Buffer.from(searchParams.get("params") ?? "", "base64").toString(),
  );

  const { title, description, logo, repository } = params;

  if (!title) {
    return new ImageResponse(<>Missing required title field.</>, {
      width: 1200,
      height: 630,
    });
  }
    ...
    return new ImageResponse(
      <div
        style={{
          ...
          gap: 10,
          backgroundImage:
            "url(https://static.invertase.io/assets/docs.page/docs.page-og-image.jpg?v=2)",
      ...
      >
        ...
        <div style={{ display: "flex", gap: 20, marginTop: 30 }}>
          <div style={{ fontSize: 40, fontWeight: 400 }}>{title}</div>
        </div>
        {!!description && (
          <div style={{ display: "flex", gap: 20 }}>
            <div style={{ fontWeight: 300, fontSize: 26, opacity: 0.8 }}>
              {description}
            </div>
          </div>
        )}
        {!!repository && (
          <div
            style={{ display: "flex", gap: 10, marginTop: 60, marginLeft: 5 }}
          >
            <svg
              fill="white"
              viewBox="0 0 100 100"
            ...
            <div
              style={{ fontWeight: 400, fontSize: 16, opacity: 0.8, top: -1 }}
            >
              {repository}
            </div>
          </div>
