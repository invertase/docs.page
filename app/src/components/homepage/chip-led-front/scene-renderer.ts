...
const DIRECT_TRIANGLE_ABSORPTION = 1.45;
const DIRECT_TRIANGLE_FALLOFF_POWER = 1;
const DIRECT_TRIANGLE_INTENSITY_SCALE = 82;
...
export function createHeroRenderer(
  ...
  frame(gpu, (currentFrame: Frame) => {
    currentFrame.pass({ target: noiseTarget }, (pass: FramePass) =>
      pass.draw(noiseDraw),
    );
  });
  ...
  return {
    ...
    renderFrame(currentFrame, { time }) {
      ...
      const floorBundle =
        current.floorBundle ?? recordFloorBundle(current, outputTarget);
      currentFrame.pass(
  ...
  function recordFloorBundle(current: RendererParts, colorTarget: Target) {
    ...
    const result = bundle(
      gpu,
      {
        target: { colors: [colorTarget.format] },
        label: "triangle-led-front-dark-floor",
      },
  ...
  function buildParts(
    ...
    setHeroSceneScale(resolveHeroSceneScale(1));
    ...
    const presentationSize = normalizedSize(
      css.width * css.dpr,
      css.height * css.dpr,
    );
  ...
  function destroyParts(current: RendererParts) {
    ...
    const texture =
      current.raycastTarget.color?.gpu ?? current.raycastTarget.gpu;
    texture?.destroy?.();
  }
}
...
function directTriangleRaycastUniformData(simulationSize: RenderSize) {
  ...
  return {
    hex_center_r: [hex.center.x, hex.center.y, hex.halfWidth, hex.halfHeight],
...
function floorUniformData(parts: RendererParts) {
  ...
  return {
    ...
    triangle: [hex.centerX, hex.centerY, hex.circumradius, hex.fillet],
    radiance_fit: [
      transform.originX,
      transform.originY,
...
function presentationHexParams(
  ...
  return {
    ...
    halfWidth: geometry.halfWidth * transform.scale,
    halfHeight: geometry.halfHeight * transform.scale,
  };
}
