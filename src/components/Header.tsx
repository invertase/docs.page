...
import { ReferenceType } from '../utils/properties';
    ...
    <div className="flex items-center">
      ...
      {properties.ref.type !== ReferenceType.base && (
        <div className="pl-6">
          <ExternalLink
            href={(() => {
              if (properties.ref.type === ReferenceType.branch) {
                return `https://github.com/${repo}/tree/${properties.ref.name}`;
              }

              if (properties.ref.type === ReferenceType.pullRequest) {
                return `https://github.com/${repo}/pull/${properties.ref.source}`;
              }

              if (properties.ref.type === ReferenceType.commit) {
                return `https://github.com/${repo}/commit/${properties.ref.name}`;
              }
            ...
            className={cx(
              ...
              {
                'bg-green-600 hover:bg-green-500 ': properties.ref.type === ReferenceType.branch,
                'bg-blue-600 hover:bg-blue-500 ': properties.ref.type === ReferenceType.pullRequest,
                'bg-pink-600 hover:bg-pink-500 ': properties.ref.type === ReferenceType.commit,
              },
            )}
          >
            {properties.ref.type === ReferenceType.branch && <Branch size={16} />}
            {properties.ref.type === ReferenceType.pullRequest && <PullRequest size={16} />}
            {properties.ref.type === ReferenceType.commit && <Commit size={16} />}
            <span className="pl-1">
              {properties.ref.type === ReferenceType.commit && properties.ref.name.slice(0, 7)}
              {properties.ref.type !== ReferenceType.commit && (
                <>
                  {properties.ref.name.slice(0, 25)}
                  {properties.ref.name.length > 25 ? '...' : ''}
                </>
              )}
            </span>
          </ExternalLink>
