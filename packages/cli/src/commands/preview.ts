...
import { parseMdx, type ParseMdxResult } from "@docs.page/mdx-bundler";
...
function startPreviewServer({
  ...
  async function sendResponse(
    ...
    let bundle: ParseMdxResult | null = null;
    ...
    if (!error && snapshot.markdown) {
      ...
      try {
        bundle = await parseMdx(snapshot.markdown, {
        ...
        try {
          ...
          socket.send(
            JSON.stringify({
              type: "response",
              revision: revision + 1,
              pathname: toPathname(socket.data.docPath),
              filePath: null,
              config: {
                json: null,
                yaml: null,
              ...
              rootDir,
              at: new Date().toISOString(),
            } satisfies PreviewResponseEvent),
          );
          revision += 1;
          return;
        }
...
function resolveHeaderDepth(config: { json: string | null; yaml: string | null }) {
  ...
  if (config.json) {
    parsedConfig = JSON.parse(config.json);
  } else if (config.yaml) {
    parsedConfig = yaml.load(config.yaml);
  }
