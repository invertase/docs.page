...
export function GitHubMeta() {
  ...
  const repository = ctx.preview ? 'preview/preview' : `${ctx.owner}/${ctx.repository}`;
  const url = ctx.preview ? '#' : `https://github.com/${repository}`;
  ...
  return (
    ...
    >
      <svg
        ...
        className="size-6 opacity-[.8] group-hover:opacity-100 transition-opacity duration-200 ease-in-out"
      >
        <path
          fill="currentColor"
      ...
      <div className="space-y-0.5">
        ...
        <div className="flex items-center gap-2 opacity-50 group-hover:opacity-100 transition-opacity duration-200 ease-in-out">
          <div className="flex items-center gap-1">
            <StarIcon size={12} />
            <code className="text-[10px]">{ctx.bundle.stars.toLocaleString()}</code>
          </div>
          <div className="flex items-center gap-1">
            <GitForkIcon size={12} />
            <code className="text-[10px]">{ctx.bundle.forks.toLocaleString()}</code>
          </div>
        </div>
      </div>
    </a>
