...
The path (either absolute or relative to the `/docs` directory) to the logo of your project, used
when dark mode is enabled. If not provided, but a `logo` is, the `logo` will be used instead.
</Property>
...
or [Brand Icons](https://fontawesome.com/search?o=r&s=solid&f=brands), for example:
  ...
  <div>
    <div style={{ display: 'flex', alignItems: 'center' }}>
      <div style={{ flexGrow: 1 }}>
        ...
        {!!props.default && <span style={{ paddingLeft: 8 }}>Default: ${props.default}</span>}
      </div>
      <div>
