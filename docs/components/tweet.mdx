---
title: Tweet
description: Embed an X post by tweet ID with optional link cards and thread context.
---
...
| --- | --- | --- | --- |
| `id` | `string` | — | Numeric tweet ID from the post URL |
| `cards` | `boolean` | `false` | Show link preview cards inside the tweet |
| `conversation` | `boolean` | `false` | Show thread replies alongside the tweet |
...
| Script loaded and `id` set | Creates the tweet widget inside a centred container |
...
Embeds are created with **Do Not Track** (`dnt: true`) enabled. Twitter receives less tracking data for embedded posts than it would with DNT disabled. Readers who block third-party scripts or cookies may still see an empty area instead of the tweet.

<Info>
  Embedded tweets load content from X's servers. Consider your audience and any privacy policies that apply when embedding third-party social content.
</Info>
