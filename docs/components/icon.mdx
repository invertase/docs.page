---
title: Icon
description: Font Awesome Icon slugs for inline graphics in prose, cards, and steps.
---
...
Place `<Icon>` inside a sentence for emphasis or visual cues. Omit `size` to match surrounding text.

Add a <Icon name="star" className="text-amber-500" /> to your favourites, or open <Icon name="folder-open" /> from the sidebar.
...
The `<Card>` component accepts an `icon` prop that renders a header icon using the same slug format. See [Card](/components/card) for layout options.
...
Use the `icon` prop on `<Step>` to replace the default step number with a custom icon. Useful when a procedure step has a clear visual association.
...
<Steps>
  ...
  <Step title="Configure the project" icon="cog">
...
| --- | --- | --- | --- |
| `name` | `string` | Yes | Font Awesome icon slug without the `fa-` prefix |
| `size` | `number` | No | Icon size in pixels; defaults to the parent font size |
...
`<Icon>` renders a native `<i>` element. Any prop not consumed by `name` or `size` is forwarded to that element—`className`, `style`, `aria-hidden`, `title`, and others.
...
- [Card](/components/card) — `icon` prop on card headers
- [Steps](/components/steps) — `icon` prop on individual steps
- [Components overview](/components/index) — when to use icons, cards, and callouts
