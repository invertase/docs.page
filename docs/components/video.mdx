...
| Condition | Result |
| --- | --- |
| `src` set | Renders an HTML5 `<video>` with one `<source>` child |
...
The component applies default layout styles: full width, 16:9 aspect ratio, and rounded corners. It does not accept `muted`, `autoPlay`, `controls`, `loop`, or other native `<video>` attributes—only `src` and `type` are implemented.
