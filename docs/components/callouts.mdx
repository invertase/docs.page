...
Use `<Info>`, `<Warning>`, `<Error>`, and `<Success>` to emphasize content readers should notice. Each callout renders a colored panel with a fixed title and icon. Put any MDX content—paragraphs, lists, or code blocks—inside the component.
...
Callouts work inside other MDX components. Use them within `<Tabs>`, `<Accordion>`, `<Steps>`, and similar containers when a panel or step needs emphasized detail.
