---
title: Code blocks
description: Fenced Markdown code with Shiki highlighting, titles, diff and focus annotations, and a copy button.
---
...
There is no `<CodeBlock />` component—standard Markdown fences are all you need.
...
If the tag is missing or unrecognised, the block renders as plain text.
...
Shiki notation transformers add diff markers, line highlights, and focus effects. Add a comment at the end of each annotated line using the `// [!code …]` pattern (or the equivalent comment syntax for your language).
...
| Condition | Result |
| --- | --- | --- |
| No language tag | Renders as `text` with no syntax colouring |
| Unrecognised language tag | Falls back to `text` |
