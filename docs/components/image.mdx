---
title: Image
description: Add screenshots and diagrams with zoom, captions, and light or dark variants.
---
...
Markdown images render as `<Image>` without extra properties. They inherit zoom behaviour from [`content.zoomImages`](/reference/docs-json-reference) in `docs.json` but cannot set `caption`, `theme`, or per-image `zoom` without switching to the component.
...
- [Video](/components/video) — self-hosted video (no repo-relative path support)
- [Components overview](/components) — when to use images, callouts, and embeds
