...
Markdown images render as `<Image>` without extra properties. They inherit zoom behavior from [`content.zoomImages`](/reference/docs-json-reference) in `docs.json` but cannot set `caption`, `theme`, or per-image `zoom` without switching to the component.
...
By default, images appear in both light and dark modes. Set `theme` to show a variant only in the matching color scheme—useful for logos or screenshots that do not work on every background.
