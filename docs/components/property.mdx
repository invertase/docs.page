---
title: Property
description: Property name, type, and required labels for API and configuration reference fields.
---

Use `<Property>` to document a single field, parameter, or prop with a name, type badge, and optional **required** or **optional** labels. Put the description in the component body so you can use full MDX—code blocks, lists, callouts, and nested components.
...
Many reference pages combine both: a summary table at the top of a props section, then `<Property>` blocks underneath for fields that need richer explanation. See [Tabs](/components/tabs) or [Accordion](/components/accordion) for that pattern.
...
| `type` | `string` | No | Type label shown as a badge |
| `required` | `boolean` | No | Shows a **required** badge |
| `optional` | `boolean` | No | Shows an **optional** badge |
| `children` | `MDX` | No | Rich description content |
...
<Property name="optional" type="boolean" optional>
  ...
  Use for fields readers may omit. Pair with prose in the description for defaults or fallback behaviour.
</Property>

---

<Property name="children" optional>
  MDX content rendered as the field description. Supports paragraphs, fenced code blocks, lists, callouts, and nested components such as `<Accordion>`.
...
- [Components overview](/components/index) — when to use property blocks, tables, and accordions
