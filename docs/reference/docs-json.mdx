...
| `scripts`, `social`, `og` | [scripts](#scripts), [social](#social), [og](#og) |
...
<Property name="scripts.googleSiteVerification" type="string">
  Google site verification token (the `content` value from the HTML tag Google Search Console gives you, not the whole tag). Rendered server-side into the page head as `<meta name="google-site-verification" content="..." />`.

  Only rendered when the page is served from your repository's **default branch** — never on a [branch preview](/features/branch-preview), tag, commit or PR preview, and never in [local preview](/features/local-preview). Ownership of a URL cannot be claimed from a ref that anyone can open a pull request for. See [Verify ownership](/customize/seo#verify-ownership-with-google-search-console).
</Property>
...
<Property name="agent.models" type="object">
  Model id to use for each provider, replacing the default docs.page assigns. Valid keys are `openai`, `anthropic`, `google`, and `xai`; only the entry matching the provider registered with `docs agent create` is read. See [Override the model](/ai-agents/ask-ai#override-the-model).
</Property>
