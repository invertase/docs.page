...
<Accordion title="theme fields">
  ...
  <Property name="theme.defaultTheme" type="&quot;light&quot; | &quot;dark&quot;">
    **Schema only** — validates in `docs.json` but is not yet applied by the theme provider. Readers currently get the platform default until default-theme support ships.
  </Property>
</Accordion>
...
<Accordion title="header fields">
  ...
  <Property name="header.showThemeToggle" type="boolean" optional>
    **Schema only** — validates in `docs.json` but the theme toggle currently always renders on hosted sites.
  </Property>

  <Property name="header.showGitHubCard" type="boolean" optional>
    **Schema only** — validates in `docs.json` but the GitHub link currently always renders on hosted sites.
  </Property>
  ...
  <Property name="header.links" type="array">
    ...
    | `locale` | string | **Schema only** — locale filter is not yet applied in the header renderer |
  </Property>
</Accordion>
...
See [Tabs](/features/navigation-tabs).
...
See [Sidebar](/features/sidebar) and [Organise](/authoring/organise).
...
See [Links](/features/navigation-links).
...
<Property name="scripts.plausible" type="string | boolean">
  Plausible analytics. Set to `true` for the hosted Plausible script, or a string URL for self-hosted Plausible. Requires a mapped [custom domain](/features/custom-domains) or [vanity subdomain](/features/vanity-subdomains) — inactive on the default `docs.page/{owner}/{repo}` path.
</Property>
...
<Property name="agent.limits.ip" type="number">
  Maximum chat requests per IP per day. Default: `200`.
</Property>

<Property name="agent.limits.repo" type="number">
  Maximum chat requests per repository per day. Default: `10000`.
</Property>
...
- [CLI](/reference/cli-reference) — `docs init`, `docs preview`, `docs check`, agent commands
