...
<Steps>
  <Step title="Provision credentials with the CLI">
    **Register the agent** — from any directory, run create with your repository and provider. The CLI encrypts your API key locally, verifies GitHub admin access, and registers the agent with docs.page.
    ...
    For every flag, provider list, and error message, see [`docs agent create`](/reference/cli-reference#docs-agent-create) in the CLI. To rotate keys, pass `--force` on a second create run.
  </Step>
...
- [CLI](/reference/cli-reference#docs-agent-create) — flags and errors for `docs agent create` and `docs agent delete`
