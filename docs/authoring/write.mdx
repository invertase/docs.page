...
5. **Expose the page to readers** — add it to the sidebar in [Organise](/authoring/organise), then confirm in [Preview](/authoring/preview).
...
**Images** — commit under `docs/` and reference with a leading slash (`/assets/dashboard.png`). Use [`<Image>`](/components/image) when you need captions, light/dark variants, or sizing.
