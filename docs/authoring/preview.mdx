...
<Steps>
  <Step title="Open a terminal at your project root">
    Use the folder that contains `docs.json`.
  </Step>
  ...
  <Step title="Change port or skip opening the browser">
    ```bash
...
<Steps>
  <Step title="Scan the project for problems">
    ```bash
    npx @docs.page/cli check
    ```

    By default, all categories report at **error** severity. The CLI validates:

    - **Internal links** — paths that must resolve to another page or asset in the repo
    - **Assets** — images and other referenced files that must exist on disk
    - **External links** — URLs the CLI can reach over the network
    - **Frontmatter paths** — `redirect`, `next`, and `previous` values that must point at valid pages
  </Step>
  <Step title="Fix each reported issue">
  ...
  <Step title="Downgrade noisy rules">
    ```bash
  ...
  <Step title="Point at a monorepo subdirectory">
