...
Run commands from the directory that contains `docs.json`. For flag-level detail and exit codes, see [CLI](/reference/cli-reference).
...
<Steps>
  ...
  <Step title="Tune severity when external checks are noisy">
    ...
    Each of `--external-links`, `--internal-links`, and `--assets` accepts `off`, `warn`, or `error`. See [CLI — check](/reference/cli-reference#docs-check) for defaults and exit codes.
  </Step>
