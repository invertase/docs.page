...
- [Organise](/authoring/organise) familiarity with sidebar groups and `href` values
...
| --- | --- | --- |
| Send readers to a default locale | `redirect` frontmatter on `docs/index.mdx` | [Redirect pages](/authoring/redirect-pages) |
...
<Steps>
  <Step title="Pick ISO 639-1 locale codes">
    Use two-letter keys (for example `en`, `fr`, `de`). They become URL segments and `sidebar` object keys.
  </Step>
  <Step title="Mirror page paths across locale folders">
    Each translated page lives under a locale folder in `docs/`:
...
<Steps>
  ...
  <Step title="Author each translated page with matching paths">
    Copy or write each page as its own `.mdx` or `.md` file. Match the path segment after the locale key across languages so the same sidebar `href` resolves in each locale:

    - English: `docs/en/installation.mdx` → sidebar `href`: `/installation`
    - French: `docs/fr/installation.mdx` → sidebar `href`: `/installation`
  </Step>
  <Step title="Set frontmatter in the target language">
  ...
  <Step title="Retire single-locale files when you switch layouts">
    If `docs/installation.mdx` used to serve `/installation`, relocate content into `docs/en/installation.mdx` (and other locales) and add [redirects](/authoring/redirect-pages) for old URLs if they were already published. To send `/` to a default locale, add `redirect: /en` (or your chosen locale) on `docs/index.mdx`.
  </Step>
</Steps>
...
<Steps>
  <Step title="Replace the flat sidebar array with locale keys">
  ...
  <Step title="Add a default tree for locale-free root URLs">
  ...
  <Step title="Align each locale tree with its files">
    Every `href` in a locale's sidebar must resolve to a page under `docs/{locale}/`. Use [Organise](/authoring/organise) for groups, nesting, tabs, and ordering within each tree.
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Run check on the project">
    ```bash
...
- [Organise](/authoring/organise) — Structure groups, tabs, and page links before you split them by locale.
- [Redirect pages](/authoring/redirect-pages) — Send `/` or renamed URLs to a default locale or new path.
