...
The first step is to point your domain at our proxy service. Using your domain provider, create a
DNS record:

| invertase.dev |  record type  |      value      |
| :-----------: | :-----------: | :-------------: |
|       @       |     CNAME     | proxy.docs.page |

Subdomains are also supported:

| invertase.dev |  record type  |      value      |
| :-----------: | :-----------: | :-------------: |
|     melos     |     CNAME     | proxy.docs.page |

If you are using a proxy service (e.g. Cloudflare), ensure you disable the proxy to allow the docs.page
proxy service to provide SSL.
...
Create a new line, adding your domain followed by the GitHub organisation and repository, for example:
