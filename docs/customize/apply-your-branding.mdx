...
For how presets, fonts, and mode switching work, see [Theme](/features/theme). Field names and defaults live in [docs.json reference](/reference/docs-json-reference).
...
The schema accepts `defaultTheme` (`light` or `dark`) to pin the starting mode, but that field is not yet wired to the theme provider—readers still get the platform default until it ships.
...
Tag each sidebar group with `"tab": "docs"` (or another tab `id`). Groups without `tab` appear on every tab. See [Navigation tabs](/features/navigation-tabs) for URL-based active tab resolution and [Organise](/authoring/organise) when you restructure navigation.
