...
| Google Search Console will not verify through GTM | Google's verifier reads raw HTML and never runs your container | Set `scripts.googleTagManager.verification` for a server-rendered tag; see [Verify ownership](/customize/seo#verify-ownership-with-google-search-console) |
