...
| Google Search Console will not verify through GTM | Google's verifier reads raw HTML and never runs your container | Set `scripts.googleSiteVerification` for a server-rendered tag on your default branch; see [Verify ownership](/customize/seo#verify-ownership-with-google-search-console) |
