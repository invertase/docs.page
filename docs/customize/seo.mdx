...
Search Console asks you to prove ownership before it shows data or accepts a sitemap. Pick the **HTML tag** method, copy the token out of the tag it gives you (the `content` value, not the whole tag), and add it to `docs.json`:
...
docs.page renders `<meta name="google-site-verification" content="..." />` into the page head on the server, so the tag is in the HTML of the first response. Merge the change into your default branch, confirm the tag in **View source**, then press **Verify**.

<Warning>
  Do not add the verification tag through Google Tag Manager. Google's verifier fetches the raw HTML and does not run JavaScript, so a tag that GTM injects at runtime is never seen and verification always fails. That is what the server-rendered `scripts.googleSiteVerification` field is for.
</Warning>

<Info>
  The tag is only rendered when a page is served from your repository's **default branch**. It does not appear on a [branch preview](/features/branch-preview), a tag, commit or PR preview, or in [local preview](/features/local-preview) — anyone can open a pull request against a public repository and preview their own branch, so honoring the token from an arbitrary ref would let a stranger have Google confirm them as an owner of your docs URL.
</Info>

On the default `docs.page/{owner}/{repo}` path you share the domain with every other docs.page site, so you can only verify a **URL prefix** property for your own path (`https://docs.page/{owner}/{repo}`), never the `docs.page` domain. To verify a domain property, [request a custom domain](/customize/custom-domain) first.
...
| Google Search Console verification fails | Tag added inside GTM (never in the raw HTML), token not on the default branch, or wrong property type | Set `scripts.googleSiteVerification` in `docs.json` and merge it to your default branch; verify a URL prefix property on the default docs.page path |
