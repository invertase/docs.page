...
Search Console asks you to prove ownership before it shows data or accepts a sitemap. Pick the **HTML tag** method, copy the token from the tag it gives you, and add it to `scripts.googleTagManager` in `docs.json`:
...
```json
{
  "scripts": {
    "googleTagManager": {
      "id": "GTM-XXXXXXX",
      "verification": "aBcD1234exampleToken"
    }
  }
}
```

`id` is your Google Tag Manager container ID and keeps working exactly as before; `verification` is the token only. docs.page renders `<meta name="google-site-verification" content="..." />` into the page head on the server, so it is in the HTML on first response. Push the change to the branch docs.page serves, confirm the tag in **View source**, then press **Verify**.

<Warning>
  Do not add the verification tag through your GTM container. Google's verifier fetches the raw HTML and does not run JavaScript, so a tag that GTM injects at runtime is never seen and verification always fails. That is what the server-rendered `verification` field is for.
</Warning>
...
| Google Search Console verification fails | Tag added inside GTM (never in the raw HTML), or wrong property type | Set `scripts.googleTagManager.verification` in `docs.json`; verify a URL prefix property on the default docs.page path |
