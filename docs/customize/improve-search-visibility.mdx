...
- Site identity fields in `docs.json` — see [Apply your branding](/customize/apply-your-branding) for `name`, logo, and related settings that feed share cards
- Optional: a [custom domain](/customize/connect-a-custom-domain) when you want canonical URLs on your own host
...
<Steps>
  ...
  <Step title="Control the default share-card image">
    ...
    When `socialPreview` is unset, docs.page generates a card from the resolved title, description, and logo (from [Apply your branding](/customize/apply-your-branding) or `og.logo`). Set `socialPreview` to `false` to omit a default image entirely.
  </Step>
...
Field types and defaults: [docs.json reference](/reference/docs-json-reference).
...
On a [custom domain](/customize/connect-a-custom-domain), the same paths sit at the domain root. The sitemap includes every page file in the tree — it does not filter by sidebar visibility or per-page `noindex`.
...
5. Optional: submit the sitemap URL in [Google Search Console](https://search.google.com/search-console) after you connect a custom domain or confirm your production URL.
...
| Share preview shows the wrong image | Page lacks `image`; `socialPreview` unset | Set frontmatter `image` or `socialPreview` in `docs.json`; confirm logo paths from [Apply your branding](/customize/apply-your-branding) |
...
- [Apply your branding](/customize/apply-your-branding) — `name`, logo, and theme settings that feed share cards
- [Connect a custom domain](/customize/connect-a-custom-domain) — serve docs and crawler files on your own domain
- [Page frontmatter](/reference/page-frontmatter) — per-page `title`, `description`, `image`, and `noindex`
- [docs.json reference](/reference/docs-json-reference) — `name`, `description`, `socialPreview`, and `seo.noindex`
