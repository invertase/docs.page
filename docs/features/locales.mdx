...
When `sidebar` is an object, docs.page derives the list of available locales from its keys (every key except `default`). That list drives URL parsing, sidebar selection, link prefixing, and the language switcher in the site chrome.
...
See [Update moved pages](/authoring/update-moved-pages) for redirect behavior and URL formats.

## Related

<CardGroup cols={2}>
  <Card title="Translate documentation" icon="language" href="/authoring/translate-documentation">
