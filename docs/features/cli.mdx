...
<Steps>
  ...
  <Step title="Skip hosts that block automated requests">
    Some hosts answer a link checker with a bot gate instead of the page. Those responses are always reported as warnings, never errors, so they cannot fail CI on their own.

    To drop a host from external checking entirely, list it:

    ```bash
    npx @docs.page/cli check --ignore-external-hosts npmjs.org,stackoverflow.com
    ```

    Declare the same hosts in `docs.json` so CI does not have to repeat the flag:
    ...
    The flag and the config value are unioned. Entries match on hostname and cover subdomains, so `npmjs.org` also covers `www.npmjs.org`. Skipped links appear as `skip` in the output and in the summary count.
  </Step>
...
| Symptom | Likely cause | Fix |
| --- | --- | --- |
| External link failures in CI | Target is temporarily down | Set `--external-links warn` or `off` for that pipeline |
| Repeated warnings from one host | Host blocks automated requests | Add it to `--ignore-external-hosts` or `check.ignoreExternalHosts` |
