...
Readers switch modes from the **theme toggle** in the site header. The `header` schema also defines `showThemeToggle`, but that field is **schema only** — the toggle always renders on hosted sites today. See [Header](/features/header) and [docs.json — Schema-only fields](/reference/docs-json-reference#schema-only-fields).
