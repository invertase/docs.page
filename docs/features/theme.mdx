...
Readers switch modes from the **theme toggle** in the site header. The `header` schema also defines `showThemeToggle`, but that field is **not yet applied** by the header renderer—the toggle currently always renders on hosted sites. See [Header](/features/header) for header chrome details.
...
Use `primary` for accent color across links, buttons, and interactive chrome. When light and dark modes need different accents, set `primaryLight` and `primaryDark`; each falls back to `primary` when omitted. Set `backgroundLight` and `backgroundDark` to adjust page background per mode.
