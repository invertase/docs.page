...
Add `previous`, `next`, or both to a page's YAML frontmatter to replace the inferred link for that direction. Paths are site-root relative, same as sidebar `href` values:
