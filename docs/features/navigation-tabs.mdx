...
That is different from the **`<Tabs>` MDX component**, which creates switchable panels inside a page. Site tabs partition the whole site chrome — header labels and sidebar contents — not inline content.
...
For every field on `tabs` entries, including optional `locale`, see [docs.json reference](/reference/docs-json-reference).
...
```json
{
  "sidebar": [
    ...
    },
    {
      "group": "Reference",
      "tab": "reference",
      "pages": [{ "title": "docs.json reference", "href": "/reference/docs-json-reference" }]
    }
  ]
}
```
...
<CardGroup cols={2}>
  ...
  <Card title="Organize your navigation" icon="sitemap" href="/authoring/organize-your-navigation">
