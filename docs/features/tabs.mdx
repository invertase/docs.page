...
The platform compares the current doc path against each tab's `href`. The tab whose path prefix best matches the current page wins. Longest match takes priority. A reader on `/features/public-github-hosting` activates the tab whose `href` is `/features`, not the tab rooted at `/`.
