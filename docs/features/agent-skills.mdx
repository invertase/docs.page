...
3. **Connect an MCP client.** MCP is on by default. Open your live site, choose **Install with MCP** from the [page action menu](/features/github-hosting#page-actions) or **Install MCP** from **Cmd+K**, and add the endpoint to Cursor, Claude Code, or another HTTP MCP client. See [MCP client](/ai-agents/mcp-server) for endpoint URLs and client setup.
