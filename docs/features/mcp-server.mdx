...
Omit the `mcp` block—or leave `enabled` unset—to keep the server available. Field defaults and related settings are documented in [Reference](/reference).

### Connect a client

Open your live docs site and use **Install MCP** from the command palette or page actions menu. The dialog copies the exact endpoint URL and client-specific config for your repository.
...
Use the URL from **Install MCP** for your site. Claude authenticates and registers the remote server for your workspace.

  </TabItem>
</Tabs>

Other MCP clients—including VS Code and Antigravity—can connect to the same HTTP URL. **Install MCP** includes copy-ready snippets for those editors as well.
