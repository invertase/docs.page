...
A third top-level field, **`socialPreview`**, sets a default preview image for every page. Per-page frontmatter can override that image. Together, these settings control outbound sharing and the small slice of site chrome reserved for community links.

Field names, types, and defaults live on the [docs.json reference](/reference/docs-json-reference). This page explains how the pieces fit together.
