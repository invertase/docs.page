...
Read [user-journeys.md](user-journeys.md) **first** for journey-first group ordering.
...
≤ 7 **leaf pages** in a single sidebar group. If more (common with component catalogs), nest into sub-groups. See [component-grouping.md](component-grouping.md).
...
5. **Validate** depth, group count, unique hrefs, icons present
6. **Sync** — `outline.pages` entry for every sidebar leaf (include `icon`)
...
Nest under a **Components** group. When > 7 component leaves, **require** nested sub-groups — see [component-grouping.md](component-grouping.md).
...
- Flat list of > 7 components in one group without sub-groups
