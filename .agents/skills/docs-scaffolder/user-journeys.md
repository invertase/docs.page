...
- **Merge capabilities by user goal**, not only by `kind`. Multiple `features.json` entries that serve the same goal (e.g. preview + local dev + file watcher → one “Local preview” how-to) belong on one page.
...
| `features.json` signals | Likely `userGoal` |
| --- | --- |
