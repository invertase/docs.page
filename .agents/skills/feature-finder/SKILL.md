...
description: >-
  ...
  features.json. Use when building a feature inventory, cataloging APIs/CLI/config
  surface area, refreshing features.json, or when the user mentions feature-finder,
  feature inventory, or code-derived documentation metadata.
---

# Feature Finder

Catalog **what the product does** by reading source code. Output is `features.json` — a code-derived inventory for downstream tools (e.g. docs-scaffolder). It does **not** decide where capabilities appear in documentation.

## Pipeline position

```
feature-finder  →  features.json       ← you are here
...
- Create or incrementally update `features.json`
...
2. Else if `features.json` already exists in the repo, update that file.
3. Else write `features.json` next to the skill that consumes it or at the repo root if unclear — ask once.
...
- [ ] 5. Write — save features.json; set updatedAt
```
