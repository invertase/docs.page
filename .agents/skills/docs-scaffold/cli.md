...
**Note:** `check` does not validate sidebar `href` entries against files. docs-scaffold must ensure every sidebar leaf has a matching `.mdx` stub before running `check`.
